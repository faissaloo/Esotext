...
public class AppStorageReaderThread extends Thread {
  ...
  public synchronized void read(ReadCommand command) {
  ...
  public AppStorageReaderThread(String filename) {
    ...
    while (true) {
      if (readQueue.length() > 0) {
        try {
          ...
          InputStream readStream = file.openInputStream();
          ReadCommand readCommand = (ReadCommand) readQueue.dequeue();
          
          ByteArrayPlus responseData = new ByteArrayPlus();

          int responseByte = 0;
          while ((responseByte = readStream.read()) != -1) {
            responseData.append_byte((byte)responseByte);
          }
          readStream.close();
          file.close();
          readCommand.callback(new ReadResponse(responseData.toByteArray()));
